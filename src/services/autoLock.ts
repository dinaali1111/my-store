...
const AutoLockManager = (() => {
  ...
  const start = (onLock: () => void) => {
    ...
    startAt = Date.now();
    console.log(
      "⏰ AutoLockManager.start - scheduling lock in",
      INACTIVITY_TIMEOUT
    );
    const id = setTimeout(() => {
...
export const useAutoLock = () => {
  ...
  const resetTimer = useCallback(() => {
    console.log(
      "👆 useAutoLock.resetTimer called - isAuthenticated:",
      isAuthenticated,
      "isLocked:",
      isLocked
    );
    if (!isAuthenticated || isLocked) {
      console.log(
        "⛔ useAutoLock.resetTimer ignored - not authenticated or already locked"
      );
      return;
    }
  ...
  useEffect(() => {
    console.log(
      "� useAutoLock effect - authenticated:",
      isAuthenticated,
      "locked:",
      isLocked
    );
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState: AppStateStatus) => {
      ...
      if (nextAppState.match(/inactive|background/)) {
        if (isAuthenticated) {
          console.log(
            "� App going to background - locking via AppState handler"
          );
          dispatch(setLocked(true));
        }
        AutoLockManager.clear();
